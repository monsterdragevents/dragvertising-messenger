...
export default function RealtimeMessenger() {
      ...
      const setupRealtimeAuth = async () => {
        try {
          ...
          if (!accessToken && user) {
            ...
            if (sessionError) {
              console.error('[RealtimeMessenger] Error getting session:', sessionError);
            }
            accessToken = currentSession?.access_token;
          }
          
          if (accessToken) {
            console.log('[RealtimeMessenger] Setting Realtime auth with token (length:', accessToken.length, ')');
            supabase.realtime.setAuth(accessToken);
            
            // Verify it was set
            const realtimeClient = (supabase.realtime as any).socket;
            if (realtimeClient) {
              console.log('[RealtimeMessenger] Realtime client exists, auth should be set');
            }
          } else {
            console.warn('[RealtimeMessenger] No access token available for Realtime auth');
            console.warn('[RealtimeMessenger] User:', user?.id, 'Session:', !!session);
          }
        } catch (error) {
          console.error('[RealtimeMessenger] Error setting Realtime auth:', error);
        }
      };
  ...
  const loadMessages = useCallback(async (conversationId: string) => {
    ...
    try {
      console.log('[RealtimeMessenger] loadMessages: Starting query for conversation:', conversationId);
      console.log('[RealtimeMessenger] loadMessages: Current user:', user?.id);
      console.log('[RealtimeMessenger] loadMessages: Current universe:', universe?.id);
      ...
      if (error) {
        ...
        console.error('[RealtimeMessenger] loadMessages: Error code:', error.code);
        console.error('[RealtimeMessenger] loadMessages: Error message:', error.message);
        console.error('[RealtimeMessenger] loadMessages: Error details:', JSON.stringify(error, null, 2));
        console.error('[RealtimeMessenger] loadMessages: This might be an RLS policy issue. Check if user has access to this conversation.');
        toast.error(`Failed to load messages: ${error.message || error.code || 'Unknown error'}`);
        setMessages([]);
        setIsLoading(false);
      ...
      if (data && data.length > 0) {
        console.log('[RealtimeMessenger] loadMessages: First message sample:', {
          id: data[0].id,
          conversation_id: data[0].conversation_id,
          sender_profile_universe_id: data[0].sender_profile_universe_id,
          content: data[0].content?.substring(0, 50),
          created_at: data[0].created_at,
          has_profile: !!data[0].profile_universes
        });
        if (data.length > 1) {
          console.log('[RealtimeMessenger] loadMessages: Last message sample:', {
            id: data[data.length - 1].id,
            conversation_id: data[data.length - 1].conversation_id,
            sender_profile_universe_id: data[data.length - 1].sender_profile_universe_id,
            content: data[data.length - 1].content?.substring(0, 50),
            created_at: data[data.length - 1].created_at
          });
        }
        console.log('[RealtimeMessenger] loadMessages: All message IDs:', data.map(m => m.id));
      } else {
        console.warn('[RealtimeMessenger] loadMessages: No messages returned from query. Conversation ID:', conversationId);
        console.warn('[RealtimeMessenger] loadMessages: This conversation may be empty or RLS policies may be blocking access');
      }
      ...
      console.log('[RealtimeMessenger] Final messages array:', messagesWithReplies);
      console.log('[RealtimeMessenger] Setting messages state with', messagesWithReplies.length, 'messages');
      ...
      console.error('[RealtimeMessenger] Error stack:', error.stack);
      console.error('[RealtimeMessenger] Full error object:', error);
    ...
    const setupChannel = async () => {
      ...
      .subscribe((status) => {
        ...
        if (status === 'SUBSCRIBED') {
          ...
          console.error('[RealtimeMessenger] Current session token:', session?.access_token ? 'Present' : 'Missing');
          // Try to refresh auth once
          if (session?.access_token) {
            console.log('[RealtimeMessenger] Re-setting Realtime auth and retrying...');
            supabase.realtime.setAuth(session.access_token);
            // Don't retry automatically - let the global auth setup handle it
            // Multiple retries can cause connection issues
          }
        } else if (status === 'TIMED_OUT') {
          console.warn('[RealtimeMessenger] Channel subscription timed out - WebSocket connection may be unstable');
        }
      });
    ...
    const setupPresenceChannel = async () => {
      ...
      .subscribe(async (status) => {
        ...
        if (status === 'SUBSCRIBED') {
          ...
          console.error('[Presence] Current session token:', session?.access_token ? 'Present' : 'Missing');
          // Try to refresh auth once
          if (session?.access_token) {
            console.log('[Presence] Re-setting Realtime auth and retrying...');
            supabase.realtime.setAuth(session.access_token);
          }
          // Don't retry automatically - let the global auth setup handle it
          // Multiple retries can cause connection issues
        } else if (status === 'TIMED_OUT') {
          console.warn('[Presence] Channel subscription timed out - WebSocket connection may be unstable');
        }
      });
            ...
            {selectedConversation ? (
              ...
              <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                ...
                <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                  ...
                  {import.meta.env.DEV && (
                    <div className="p-2 bg-yellow-100 dark:bg-yellow-900/20 text-xs border-b">
                      <p>Debug: messages.length = {messages.length}</p>
                      <p>selectedConversation?.id = {selectedConversation?.id || 'null'}</p>
                      <p>universe?.id = {universe?.id || 'null'}</p>
                  ...
                  {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center flex-1 py-12">
                      ...
                      {import.meta.env.DEV && selectedConversation?.id && (
